<?php
	...
	function custom_taxonomies_terms_links($post_id) {
		...
		$terms = wp_get_object_terms( $post_id, 'product_cat', array('orderby' => 'parent', 'order' => 'DESC', 'fields' => 'all') );
	...
	function facetly_save_post($post_id) {
		...
		if($post_status == 'trash' || $post_status == 'auto-draft' || $post_type != 'product'){
			return $post_id;
		} else if ($post_status == 'publish' && $post_type == 'product') {
			try {
		...
		} else if ( ($post_status == 'pending' || $post_status == 'draft' ) && $post_type == 'product') {
